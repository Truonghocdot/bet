...
class PruneChatCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $now = now('UTC');
        $messages = DB::table('chat_messages')->where('created_at', '<', $now->copy()->subHours($messageHours))->delete();
        $audits = DB::table('chat_moderation_actions')->where('created_at', '<', $now->copy()->subDays((int) $this->option('audit-days')))->delete();
